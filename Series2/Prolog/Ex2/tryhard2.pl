...
recursiveFoo([Leaf|_], Node,_,_,_,_,_,_,_, Result) :- 
    (
...
%randomWalk(0,1,[],0,3,NewNewNode,NewNewDegree,[1,0,0],NewNewChildren,JobA,JobB).

randomWalk(_,RandomUnvisitedNode,_,_,_,_,Degree,[_, _, _],_,[0,0],0):- getAS(DegreeOfRdNode,RandomUnvisitedNode,Degree), DegreeOfRdNode =\= 2.
...
parseDegree(Index, Accum, N, Result, _DT):-
    Index =:= N,
    Result = Accum, !.
...
read_and_do(T, Stream, CurrentAnswers, Answers):-
    ...
    append(CurrentAnswers, [JobResult, JobResultList], NewCurrentAnswers),
    NewT is T - 1,
...
read_edges(_Stream, 0, CurrList, CurrList).

read_edges(Stream, M, CurrList, InputList):-
    read_line(Stream, Pair),
    append(CurrList, Pair, NewList),
    NewM is M - 1,
...
% Αυτο ειναι το άλλο παραδειγμα
%[4,6,8,1,3,4,2,8,6,7,9,3,4,7,1,5,4,9]
% Αυτο δε δουλευει σωστα
% [6,1,13,5,5,8,16,1,7,13,13,10,10,3,8,14,3,2,7,5,11,12,7,12,7,10,15,4,13,15,11,16]
dotheactualjob(N, M, Node, Degree, JobResult, JobResultList):-
    NewN is N + 1,
    parseDegree(1,[], NewN, ParseResult, Degree),
    init([], 1,  NewN, NumOfChildren),
    recursiveFoo(ParseResult, Node, Degree, NumOfChildren, [0,0,0], NewNode, NewDegree, NewNumOfChildren, NewLastSomething, ReRE),
    [LastLastFather,LastNumVisited, NumFinal]= NewLastSomething,
    getAS(LastFatherList, LastLastFather, NewNode),
    (LastNumVisited > (N - 3) ; ParseResult = [0,0] ; N =\= M ; LastFatherList = [] ; ReRE = [0,0] -> JobResultList = [0,0], JobResult = 0
